...
/*   Updated: 2024/07/18 22:41:24 by mconreau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Gateway::run(Request& req, const string &passcgi, const string &target)
{
	...
	if ((pid = ::fork()) == -1)
		 return (::close(pipe_p2c[0]), ::close(pipe_p2c[1]), ::close(pipe_c2p[0]), ::close(pipe_c2p[1]), Logger::fail("Gateway: Failed to create pipe"), 500);
...
Gateway::recv(const int &fd)
{
	...
	if (r == 0)
		return (Logger::fail("Gateway: Timeout"), 504);
	if (FD_ISSET(fd, &s))
