...
/*   Updated: 2024/08/22 19:36:44 by muteza           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int argc, char **argv) {
    ...
    std::cout << "Time to process a range of " << m.vector.size() << " elements with std::vector" <<  " : " << v_time << "us" << std::endl;

    std::clock_t d_time = clock();
    m.recursive(m.deque, 0, m.deque.size() -1);
    d_time = clock() - d_time;

    std::cout << "Time to process a range of " << m.deque.size() << " elements with std::deque" <<  " : " << d_time << "us" << std::endl;
}
